...
der Funktion \emph{curve\_ fit} \cite{scipy} aus dem Python Paket \emph{scipy.optimize}\cite{scipy} durchgeführt. %cite nach paket reicht
...
\begin{figure}[h!]
  ...
  \label{fig: T_00}
\end{figure}
In der Abbildung ist die mit Hilfe von \emph{scipy.optimize} erstelle Fit an die Funktion \eqref{} zu erkennen. %eqref
...
An die Messwerte wurde eine Funktion der Form \eqref{} gefittet, hierbei wird für %präsenz, eqref
die Funktion \emph{curve\_fit} die folgenden Startparameter gewählt
\begin{align*}
  I_{0,1}&=\SI{2.03}{\micro\ampere} & I_{0,2}&=\SI{1.68}{\micro\ampere}\\       %würde ich weglassen, wofür ist das relevant?
...
aufgelistet.
\FloatBarrier
\input{../Messdaten/table/polar_final.tex} %besser varphi
An die Messwerte wird die Funktion %kannst du auch referenziere, habe ich in der theorie
...
Bei der Messung betrug der Abstand zum Schirm $l=\SI{83}{\centi\meter}$ %präsenz
und wurde mit einem Gitter mit dem Gitterabstand von $a=\SI{1e-5}{\meter}$ durchgeführt. %präsenz
...
mit aufgelistet. Hierbei wird die Wellenlänge mit Formel \eqref{} und der Winkel mit \eqref{} berechnet. %eqref, winkelgleichung nicht explizit aufgeführt, kannst du hier nochmal hinschreiben
...
\FloatBarrier
\FloatBarrier
\subsection{Untersuchugn der Stabilitätsbedingung} %Untersuchung
Die Stabilitätsbedingung wird für zwei verschiedene Konfigurationen untersucht,
zum einen werden für den Resonator zwei konkave Spiegel verwendet und zum andern %anderen
ein konkaver und ein flacher Spiegel. %ebener
...
\FloatBarrier
\FloatBarrier
\subsubsection{Konkav-Konkave Konfiguration} %oder konfokale
